...
import sample.rdd.json.{Person, RandomRDD}
...
// The RDD will parallelize the workload across `numSlices`
 final class RandomRDD[A: ClassTag](@transient private val sc: SparkContext, numSlices: Int, numValues: Int, random: => A) extends RDD[A](sc, deps = Seq.empty) {
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[A] =
    split.asInstanceOf[json.RandomPartition[A]].values
  ...
  override protected def getPartitions: Array[Partition] =
    ((0 until slicesWithExtraItem)
      .view
      .map(new json.RandomPartition[A](_, valuesPerSlice + 1, random))
      ++ (slicesWithExtraItem until numSlices)
      .view
      .map(new json.RandomPartition[A](_, valuesPerSlice, random)))
      .toArray
...
object main {
  def main(args: Array[String]): Unit = {
    ...
    val value = rdd.toDS()
    val value1 = value.cache()
    value1.write.mode("overwrite").parquet("C:\\Users\\Барков Артур\\IdeaProjects\\SparkSimpleApp\\rdd-generated")
